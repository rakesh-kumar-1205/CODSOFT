...
class TodoListApp(tk.Tk):
    ...
    def add_task(self):
        ...
        if task != "Enter your todo here...":
            last_index = self.task_list.size() + 1
            self.task_list.insert(tk.END, f"{last_index}. {task}")
    ...
    def load_tasks(self):
        try:
            with open("tasks.json", "r") as f:
                data = json.load(f)
                for task in data:
                    self.task_list.insert(tk.END, f"{task['serial']}. {task['text']}")
    ...
    def save_tasks(self):
        ...
        for i in range(self.task_list.size()):
            task_info = self.task_list.get(i)
            serial, text = task_info.split(". ", 1)
            color = self.task_list.itemcget(i, "fg")
            data.append({"serial": serial, "text": text, "color": color})
